...
export function registerSessionCommands(program) {
    ...
    program
        ...
        try {
            ...
            if (!noteContent) {
                ...
                if (isJson)
                    console.log(JSON.stringify({ success: true, type: 'session' }));
                else
                    console.log(chalk.green('✓ Session note added.'));
            }
            else {
                ...
                if (isJson)
                    console.log(JSON.stringify({ success: true, type: 'exercise_set' }));
                else
                    console.log(chalk.green(`✓ Note added to last set of ${exercise.name}.`));
            }
        }
        catch (err) {
            if (isJson)
    ...
    program
        ...
        try {
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, status: 'completed' }));
            else
                console.log(chalk.green('✓ Workout session completed! Great job.'));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
    ...
    program
        ...
        try {
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, status: 'cancelled' }));
            else
                console.log(chalk.yellow('⚠ Workout session cancelled.'));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
    ...
    program
        ...
        try {
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, swappedSets: info.changes }));
            else
                console.log(chalk.green(`✓ Swapped ${oldExercise.name} for ${newExercise.name} (${info.changes} sets updated).`));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
    ...
    program
        ...
        try {
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, undoSetId: lastSet.id }));
            else
                console.log(chalk.green('✓ Last set undone.'));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
    ...
    program
        ...
        try {
            ...
            if (!targetSet)
                throw new Error(`Set ${setNum} not found for ${exercise.name}.`);
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, updatedSetId: targetSet.id }));
            else
                console.log(chalk.green(`✓ Updated set ${setNum} of ${exercise.name} to ${newWeight}kg x ${newReps} reps.`));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
    ...
    program
        ...
        try {
            ...
            if (isJson)
                console.log(JSON.stringify({ success: true, deletedSetId: targetSet.id }));
            else
                console.log(chalk.green(`✓ Deleted set ${setNum} of ${exercise.name}.`));
        }
        catch (err) {
            if (isJson)
                console.log(JSON.stringify({ error: err.message }));
