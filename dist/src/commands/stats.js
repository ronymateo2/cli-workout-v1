    ...
    program
        ...
        `).all(lastWorkout.id);
            ...
            if (lastWorkout.notes)
                console.log(chalk.dim(`Notes: ${lastWorkout.notes}`));
    ...
    program
        ...
        `).all(profile.id);
            const relevantInjuries = activeInjuries.filter(inj => {
                if (inj.affected_exercises) {
                    ...
                    if (affected.some(a => exercise.name.toLowerCase().includes(a) || a.includes(exercise.name.toLowerCase())))
                        return true;
                }
                if (inj.body_region && exercise.muscles) {
                    const muscles = exercise.muscles.toLowerCase().split(',').map(m => m.trim());
                    if (muscles.some(m => inj.body_region.toLowerCase().includes(m) || m.includes(inj.body_region.toLowerCase())))
                        return true;
                }
                return false;
            });
        ...
        `).all(profile.id, exercise.id);
            if (historySets.length === 0) {
                if (isJson)
                    console.log(JSON.stringify({ error: `Not enough data for ${exercise.name}` }));
                else
                    console.log(chalk.yellow(`No completed volume data found for ${exercise.name} yet. Go log some sets!`));
                return;
            }
