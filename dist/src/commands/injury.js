...
injuryCmd
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(profile.id, bodyRegion.toLowerCase(), severity, options.description || null, options.exercises ? options.exercises.toLowerCase() : null, options.notes || null);
...
injuryCmd
    ...
    try {
        ...
        if (!injury)
            throw new Error(`Injury #${id} not found for this profile.`);
        if (injury.status === 'recovered')
            throw new Error(`Injury #${id} is already marked as recovered.`);
...
injuryCmd
    ...
    try {
        ...
        if (isJson)
            ...
            console.log(chalk.green(`✓ Injury #${id} updated.`));
    }
    catch (err) {
        if (isJson)
            console.log(JSON.stringify({ error: err.message }));
