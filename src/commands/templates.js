    ...
    try {
      ...
      const insertTemplate = db.transaction((profId, tName, exercisesConfigStr) => {
        ...
        for (const configStr of configs) {
          ...
          if (!exName || !setConf) throw new Error(`Invalid format for exercise config: ${configStr}. Use "exerciseName:4x8"`);
      ...
      if (isJson) console.log(JSON.stringify({ success: true, templateId, name }));
      else console.log(chalk.green(`✓ Template '${name}' created successfully.`));
    } catch(err) {
      if (isJson) console.log(JSON.stringify({ error: err.message }));
      else console.error(chalk.red(`✗ ${err.message}`));
    }
